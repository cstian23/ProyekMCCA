...
if (isset($_POST['filter'])) {
  $rekam_medis = get("SELECT *, rm.rm_id FROM rekam_medis rm
                    INNER JOIN pasien ON rm.pasien_id = pasien.pasien_id 
                    INNER JOIN dokter ON rm.dokter_id = dokter.dokter_id 
                    INNER JOIN ruang ON rm.ruang_id = ruang.ruang_id
  ...
  echo "<script>alert('Data yang ditampilkan adalah dari 13 November 2022 sampai 13 November 2023');</script>";
  $rekam_medis = get("SELECT *, rm.rm_id FROM rekam_medis rm
                    INNER JOIN pasien ON rm.pasien_id = pasien.pasien_id 
                    INNER JOIN dokter ON rm.dokter_id = dokter.dokter_id 
                    INNER JOIN ruang ON rm.ruang_id = ruang.ruang_id
                    WHERE rm.tanggal BETWEEN '2022-11-13' AND '2023-11-13' ") or die(mysqli_error($conn));
}
                ...
                <tbody>
                  <?php foreach ($rekam_medis as $rm) : ?>
                    <tr>
                      ...
                      <td>
                        <?php
                        $obats = mysqli_query($conn, "SELECT * FROM rm_obat JOIN obat ON rm_obat.rm_obat_id = obat.obat_id WHERE rm_obat_id = '$rm[rm_id]'") or die(mysqli_error($conn));
